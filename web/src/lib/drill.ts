...
export class DrillController {
  ...
  start(): DrillState {
    ...
    const question = generateQuestion(this.state.config, this.random);
  ...
  armQuestion(): boolean {
    if (
      ...
      return false;
    if (this.clock.now() >= this.state.deadline!) {
      this.finish();
      return false;
    }
  ...
  tick(): DrillState {
    if (
      this.state.status === "running" &&
      this.clock.now() >= this.state.deadline!
    )
      this.finish();
    return this.state;
  }
  ...
  submit(raw: string): "ignored" | "accepted" | "expired" {
    ...
    const record: AnswerRecord = {
      ...
      timeTakenSec: Math.max(
        0,
        (now - this.state.currentQuestionStartedAt) / 1000,
      ),
  ...
  finish(): DrillState {
    ...
    if (!this.saved) {
      ...
      if (this.save) {
        try {
          this.save({
            sessionName: this.state.config.sessionName,
            duration: this.state.config.duration,
            insights: [...this.state.records],
          });
        } catch (error) {
  ...
  /** Leaving the page abandons a partial drill; no partial session is saved. */
  cancel(): void {
